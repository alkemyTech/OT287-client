...
const Home = ({ news, members, slider, error, errorMessage }) => (
  <Box sx={{maxWidth:'1600px', margin:'0 auto', alignContent: 'center'}}>
    ...
    <Box sx={{mr:'100px', ml:'100px', mt:'50px'}} >
       ...
       color={'#000000'}
       maxWidth={'400px'}
       sx={{position: 'relative', top: {lg: '8rem'}, margin:'0 auto', textAlign: 'center', left: {lg: '1rem'}, fontSize:{xs: 12, sm: 18}}}
       > 
       ...
       <Link style={{textDecoration:'none', color: 'inherit'}} to = '/contacto' ><Button variant="contained" sx={{top: {xs:'2px', lg:'30px'}}}>Contactanos</Button> </Link>
       </Typography>
      ...
      <Box sx={{textAlign: 'right', bottom: {xs: '12px', position: 'relative'}}}   >
        <Link style={{textDecoration:'none', color: 'inherit'}} to={'/sobre-nosotros'}> Ver staff {`>`} </Link>
       </Box>
      <MembersCards
      data={members}
      error={error}
      ...
      <Box sx={{textAlign: 'right'}}>
          <Link style={{textDecoration:'none', color: 'inherit'}} to={'/novedades'} > Ver todos {`>`} </Link>
      </Box>
      <NewsCards
      data={news}
      error={error}
